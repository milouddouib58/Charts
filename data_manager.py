...
class PDFReport(FPDF):
    ...
    def process_text(self, text):
        """
        Processes Arabic text to reshape and fix the Right-To-Left direction.
        Splits by newline to prevent the first word of a paragraph from jumping to the end.
        """
        ...
        try:
            ...
            # If the text has multiple lines, process each line individually
            if '\n' in text_str:
    ...
    def draw_analysis_section(self, narrative):
        ...
        # To prevent the first word jumping to the end of the paragraph, 
        # we process the narrative paragraph by paragraph.
        paragraphs = narrative.split('\n\n')
        for p in paragraphs:
            if p.strip():
                # multi_cell automatically handles line breaks. By passing the processed 
                # paragraph text, it should render correctly Right-to-Left.
                self.multi_cell(180, 7, self.process_text(p), 0, 'R')
                self.set_x(15) # reset X after multi_cell
                self.ln(2) # small gap between paragraphs
